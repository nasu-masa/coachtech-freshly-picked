...
Mac（特に Apple Silicon）環境では MySQL 8.0.26 が正常に動作しない既知の不具合があります。
そのため、本プロジェクトでは **MySQL 8.0.30 以降** の使用を推奨します。

Windows 環境では 8.0.26 でも動作しますが、環境差によるトラブルを避けるため、
可能であれば同じバージョン（8.0.30+）を使用してください。
